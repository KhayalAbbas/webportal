...
from app.schemas.ai_proposal import AIProposal, AIProposalSource, AIProposalCompany, AIProposalMetric
...
def transform_bundle_to_proposal(bundle: RunBundleV1) -> AIProposal:
    """
    Transform a RunBundleV1 into a valid AIProposal for Phase 2 ingestion.
    
    Args:
        bundle: The stored bundle from research_run_bundles
        
    Returns:
        AIProposal: Valid proposal for Phase 2 ingestion
        
    Raises:
        ValueError: If bundle cannot be transformed into valid proposal
    """
    # 1) Extract query (required)
    query = None
    if bundle.plan_json and isinstance(bundle.plan_json, dict):
        query = bundle.plan_json.get("objective")
    
    if not query and bundle.proposal_json and isinstance(bundle.proposal_json, dict):
        query = bundle.proposal_json.get("query")
    
    if not query:
        raise ValueError("bundle_validation_failed: No query found in bundle.plan_json.objective or bundle.proposal_json.query")
    
    # 2) Transform sources
    proposal_sources = []
    for source in bundle.sources:
        # Map RunBundleV1 source fields to AIProposalSource fields
        proposal_source = AIProposalSource(
            temp_id=getattr(source, 'temp_id', None) or f"source_{source.sha256[:8]}",
            title=getattr(source, 'title', None) or f"Document {source.sha256[:8]}",
            url=getattr(source, 'url', None),
            provider=getattr(source, 'provider', None),
            fetched_at=getattr(source, 'retrieved_at', None)
        )
        proposal_sources.append(proposal_source)
    
    # 3) Transform companies
    if not bundle.proposal_json or 'companies' not in bundle.proposal_json:
        raise ValueError("bundle_validation_failed: No companies found in bundle.proposal_json")
    
    proposal_companies = []
    companies_data = bundle.proposal_json['companies']
    
    for i, company_data in enumerate(companies_data):
        # Validate company name
        if not company_data.get('name'):
            raise ValueError(f"bundle_validation_failed: Company {i} missing required name")
        
        # Validate and transform metrics
        metrics_data = company_data.get('metrics', {})
        if not isinstance(metrics_data, list):
            # If metrics is a dict, convert to empty list for now
            # Could be enhanced to convert dict to list of metrics
            metrics_list = []
        else:
            metrics_list = []
            for metric in metrics_data:
                if isinstance(metric, dict) and 'key' in metric and 'value' in metric:
                    metric_obj = AIProposalMetric(
                        key=metric['key'],
                        type=metric.get('type', 'text'),
                        value=metric['value'],
                        currency=metric.get('currency'),
                        unit=metric.get('unit'),
                        confidence=metric.get('confidence'),
                        source_temp_id=metric.get('source_temp_id'),
                        evidence_snippet=metric.get('evidence_snippet')
                    )
                    metrics_list.append(metric_obj)
        
        # Validate evidence_snippets (min 1 required)
        evidence_snippets = company_data.get('evidence_snippets', [])
        if not evidence_snippets:
            raise ValueError(f"bundle_validation_failed: Company '{company_data['name']}' missing required evidence_snippets (min 1)")
        
        # Validate source_sha256s (min 1 required and must exist in bundle sources)
        source_sha256s = company_data.get('source_sha256s', [])
        if not source_sha256s:
            raise ValueError(f"bundle_validation_failed: Company '{company_data['name']}' missing required source_sha256s (min 1)")
        
        # Verify source_sha256s exist in bundle sources
        bundle_sha256s = {source.sha256 for source in bundle.sources}
        for sha256 in source_sha256s:
            if sha256 not in bundle_sha256s:
                raise ValueError(f"bundle_validation_failed: Company '{company_data['name']}' references unknown source_sha256 '{sha256}'")
        
        # Create proposal company
        proposal_company = AIProposalCompany(
            ...
            ai_rank=company_data.get('ai_rank'),
            ai_score=company_data.get('ai_score')
        )
        proposal_companies.append(proposal_company)
    
    # 4) Create the AIProposal
    proposal = AIProposal(
        query=query,
        sources=proposal_sources,
        companies=proposal_companies,
        generated_at=datetime.utcnow(),
        model="phase3_transformer"
    )
...
class ResearchRunService:
    ...
    async def accept_bundle(
        ...
        # If accept_only=True, perform additional validation for Phase 2 transformation
        if accept_only:
            try:
                # Test if bundle can be transformed to valid AIProposal
                transform_bundle_to_proposal(bundle)
            except ValueError as exc:
                # Re-raise with upload validation prefix for clear UI error messages
                if "bundle_validation_failed" in str(exc):
                    raise exc  # Already has the right prefix
                else:
                    raise ValueError(f"bundle_validation_failed: {str(exc)}")
        ...
        if accept_only:
            ...
            # Submit durable job for ingestion
            job_service = DurableJobService(self.db)
    ...
    async def approve_bundle_for_ingestion(
        ...
        # Submit durable job for ingestion
        job_service = DurableJobService(self.db)
        # Convert bundle to JSON-safe dict to avoid UUID serialization issues
        bundle_data = json.loads(bundle.model_dump_json())
        job_id = await job_service.enqueue_job(
            tenant_id=tenant_id,
            run_id=run_id,
            job_type="ingest_bundle",
    ...
    async def _ingest_bundle_background(
        ...
        try:
            ...
            if not run.company_research_run_id:
                run.status = "failed"
                await self.db.commit()
                raise ValueError("no_company_research_run_id: Phase 2 ingestion requires company_research_run_id")

            # Transform bundle to valid AIProposal using transformer
            try:
                proposal = transform_bundle_to_proposal(bundle)
            except ValueError as exc:
                run.status = "failed"
                await self.db.commit()
                raise ValueError(f"bundle_transform_error: {str(exc)}")
    ...
    async def _store_bundle(
        ...
        # Convert to JSON-safe dict (UUID -> str, datetime -> iso)
        bundle_json = json.loads(bundle.model_dump_json())
        json.dumps(bundle_json)  # must not throw
        
        if found:
            # Update existing bundle
            found.bundle_sha256 = bundle_sha256
            found.bundle_json = bundle_json
        else:
    ...
    async def _get_or_create_source(
        ...
        """Upsert source_documents by (tenant_id, company_research_run_id, content_hash)."""
        existing = await self.db.execute(
            select(ResearchSourceDocument).where(
                ResearchSourceDocument.tenant_id == tenant_id,
                ResearchSourceDocument.company_research_run_id == company_research_run_id,
