"""Phase 9.5 preflight: health, openapi, alembic head check, git snapshot."""
...
ARTIFACT = ROOT / "scripts" / "proofs" / "_artifacts" / "phase_9_5_preflight.txt"
OPENAPI_BEFORE = ROOT / "scripts" / "proofs" / "_artifacts" / "phase_9_5_openapi_before.json"
...
async def snapshot_alembic() -> str:
    ...
    assert head == current, f"alembic current {current} != head {head}"
