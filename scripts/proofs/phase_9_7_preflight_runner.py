...
PRE_FLIGHT = ART_DIR / "phase_9_7_preflight.txt"
OPENAPI_BEFORE = ART_DIR / "phase_9_7_openapi_before.json"
...
def run_cmd(cmd: list[str]) -> str:
    ...
    if not resolved:
        if binary.lower() == "git":
            resolved = "C:/Program Files/Git/bin/git.exe"
...
async def main() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        lines.append("GET /openapi.json (saved phase_9_7_openapi_before.json)")
        lines.append(f"status={resp.status_code} length={len(resp.text)}")

    lines.append("alembic current")
    lines.append(run_cmd(["alembic", "current"]))
    lines.append("alembic heads")
    lines.append(run_cmd(["alembic", "heads"]))
