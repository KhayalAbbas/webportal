"""Phase 9.1 discovery provider proof script.

This script:
- Runs health + OpenAPI capture (after changes)
- Creates a research run for an existing tenant
- Calls the deterministic discovery provider twice to prove idempotency
- Dumps DB excerpts (source documents, enrichments, prospects, evidence)
- Writes artifacts under artifacts/ for review
"""
...
OPENAPI_AFTER_PATH = ARTIFACT_DIR / "phase_9_1_openapi_after.json"
OPENAPI_AFTER_EXCERPT_PATH = ARTIFACT_DIR / "phase_9_1_openapi_after_excerpt.txt"
PROOF_TXT_PATH = ARTIFACT_DIR / "phase_9_1_proof.txt"
PROOF_CONSOLE_PATH = ARTIFACT_DIR / "phase_9_1_proof_console.txt"
CALLS_JSON_PATH = ARTIFACT_DIR / "phase_9_1_discovery_provider_calls.json"
FIRST_CALL_PATH = ARTIFACT_DIR / "phase_9_1_first_call.json"
SECOND_CALL_PATH = ARTIFACT_DIR / "phase_9_1_second_call.json"
DB_EXCERPT_PATH = ARTIFACT_DIR / "phase_9_1_db_excerpt.txt"
...
async def _create_run(session, tenant_id: str, role_id: UUID):
    ...
    run = await service.create_research_run(
        ...
        data=CompanyResearchRunCreate(
            ...
            name="Phase 9.1 Discovery Provider Proof",
...
async def main():
    ...
    async with AsyncClient(app=app, base_url="http://testserver") as client:
        ...
        # Write excerpt with the discovery provider endpoint + schemas
        ...
        schemas_excerpt = {
            ...
            if "DiscoveryProvider" in k
        }
        excerpt_lines = ["PATH EXCERPT", json.dumps({path_key: path_excerpt}, indent=2), "", "SCHEMAS", json.dumps(schemas_excerpt, indent=2)]
        OPENAPI_AFTER_EXCERPT_PATH.write_text("\n".join(excerpt_lines), encoding="utf-8")

        url = f"/company-research/runs/{run.id}/discovery/providers/deterministic_phase_9_1/run"
        call1 = await client.post(url, json={})
        call2 = await client.post(url, json={})
    ...
    CALLS_JSON_PATH.write_text(json.dumps(results, indent=2), encoding="utf-8")
    ...
    _ok(len(hashes) == unique_hashes, "no duplicate source content_hash values in DB excerpt")

    proof_status = "PASS" if all(a.startswith("PASS") for a in assertions) else "FAIL"

    lines = [
        "PHASE 9.1 DISCOVERY PROVIDER PROOF",
        "===================================",
        ...
        f"Calls JSON: {CALLS_JSON_PATH}",
        f"DB excerpt: {DB_EXCERPT_PATH}",
    ]
    PROOF_TXT_PATH.write_text("\n".join(lines), encoding="utf-8")
    console_output = "\n".join(lines)
    PROOF_CONSOLE_PATH.write_text(console_output, encoding="utf-8")
