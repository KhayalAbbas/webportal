...
ARTIFACT = ROOT / "scripts" / "proofs" / "_artifacts" / "phase_6_2_preflight.txt"
...
sections.append("\n== git show phase_6_1_signed_off ==")
sections.append(run_cmd([str(GIT_EXE), "show", "-s", "--oneline", "phase_6_1_signed_off"]))
...
sections.append(f"\n== GET /openapi.json status {resp.status_code} ==")
...
async def db_connectivity() -> None:
    async with get_async_session_context() as session:
        ...
        sections.append(f"SELECT 1 -> {val}")
