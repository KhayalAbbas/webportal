...
LOG_ARTIFACT = ARTIFACT_DIR / "phase_4_6_safety_limits.log"
OPENAPI_ARTIFACT = ARTIFACT_DIR / "phase_4_6_openapi.json"
PRECHECK_ARTIFACT = ARTIFACT_DIR / "phase_4_6_preflight.txt"
POSTCHECK_ARTIFACT = ARTIFACT_DIR / "phase_4_6_postcheck.txt"

LOG_LINES: list[str] = []

MAX_FETCH_BYTES_UNDER_TEST = 2048
FETCH_TIMEOUT_SECONDS_UNDER_TEST = 0.5
SLOW_DELAY_SECONDS = 1.0
BIG_BODY_BYTES = 5000


class SafetyLimitHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        path = self.path.rstrip("/") or "/"

        if path.endswith("/ok_html"):
            self._write(200, "text/html", b"<html><body>OK_HTML</body></html>")
            return
        if path.endswith("/big_html"):
            body = b"A" * BIG_BODY_BYTES
            self._write(200, "text/html", body)
            return
        if path.endswith("/slow_html"):
            time.sleep(SLOW_DELAY_SECONDS)
            self._write(200, "text/html", b"SLOW_HTML")
            return
        if path.endswith("/bin_payload"):
            body = b"\x00\x01binarypayload"
...
def start_fixture_server(port: int = 8778) -> ThreadingHTTPServer:
    server = ThreadingHTTPServer(("127.0.0.1", port), SafetyLimitHandler)
...
def fetch_db_state(tenant_id: str, run_id: str) -> dict:
    ...
    with psycopg2.connect(dsn) as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT id, url, url_normalized, status, attempt_count, next_retry_at, meta, created_at, last_error, error_message, fetched_at, content_hash, http_status_code, http_final_url, canonical_final_url, http_error_message, mime_type
...
def main() -> int:
    ...
    log("=== PHASE 4.6 SAFETY LIMITS PROOF ===")
    ...
    env_overrides = {
        "MAX_FETCH_BYTES": str(MAX_FETCH_BYTES_UNDER_TEST),
        "FETCH_TIMEOUT_SECONDS": str(FETCH_TIMEOUT_SECONDS_UNDER_TEST),
    ...
    try:
        ...
        log("Fixture HTTP server started on 127.0.0.1:8778")
        ...
        run_payload = {
            ...
            "name": f"Phase4.6-SafetyLimits-{uuid.uuid4().hex[:6]}",
            "description": "Proof safety limits (timeout/max-bytes/content-type)",
            "sector": "banking",
            "region_scope": ["US"],
        }
        ...
        ok_url = f"{base}/ok_html"
        big_url = f"{base}/big_html"
        slow_url = f"{base}/slow_html"
        bin_url = f"{base}/bin_payload"
        for idx, url in enumerate([ok_url, big_url, slow_url, bin_url], start=1):
            payload = {"title": f"Safety-{idx}", "url": url}
        ...
        assert_true(ok_url in sources_by_url and big_url in sources_by_url and slow_url in sources_by_url and bin_url in sources_by_url, "all sources present")

        ok_src = sources_by_url[ok_url]
        big_src = sources_by_url[big_url]
        slow_src = sources_by_url[slow_url]
        bin_src = sources_by_url[bin_url]

        ok_meta = (ok_src.get("meta") or {}).get("fetch_info", {})
        big_meta = (big_src.get("meta") or {}).get("fetch_info", {})
        slow_meta = (slow_src.get("meta") or {}).get("fetch_info", {})
        bin_meta = (bin_src.get("meta") or {}).get("fetch_info", {})
        ...
        ok_events = find_events("fetch_succeeded", str(ok_src.get("id")))
        too_large_events = find_events("fetch_too_large", str(big_src.get("id")))
        timeout_events = find_events("fetch_timed_out", str(slow_src.get("id")))
        unsupported_events = find_events("fetch_unsupported_content_type", str(bin_src.get("id")))

        assert_true(ok_src.get("status") == "fetched", "ok_html fetched")
        assert_true(ok_src.get("http_status_code") == 200, "ok_html HTTP 200")
        assert_true(ok_src.get("mime_type", "").startswith("text/html"), "ok_html mime_type captured")
        assert_true(len(ok_events) >= 1, "fetch_succeeded event recorded for ok_html")

        assert_true(big_src.get("status") == "failed", "big_html failed")
        assert_true(big_src.get("last_error") == "fetch_too_large", "big_html marked fetch_too_large")
        assert_true(big_meta.get("bytes_read", 0) > MAX_FETCH_BYTES_UNDER_TEST, "big_html bytes_read exceeds max")
        assert_true(len(too_large_events) >= 1, "fetch_too_large event recorded")

        assert_true(slow_src.get("status") == "failed", "slow_html failed")
        assert_true(slow_src.get("last_error") == "fetch_timeout", "slow_html marked fetch_timeout")
        assert_true(len(timeout_events) >= 1, "fetch_timed_out event recorded")

        assert_true(bin_src.get("status") == "failed", "bin_payload failed")
        assert_true(bin_src.get("last_error") == "unsupported_content_type", "bin_payload marked unsupported_content_type")
        http_info_bin = (bin_meta.get("http") or {})
        assert_true((http_info_bin.get("content_type") or "").startswith("application/octet-stream"), "bin_payload content_type captured")
        assert_true(len(unsupported_events) >= 1, "fetch_unsupported_content_type event recorded")
