...
class AIProposalService:
    ...
    async def _ingest_company_evidence(
        ...
        for i, evidence_snippet in enumerate(company_data.evidence_snippets):
            ...
            # Check for existing evidence (avoid duplicates)
            # Identical = same tenant, prospect, snippet, and source_sha256
            existing_query = await self.session.execute(
                select(CompanyProspectEvidence).where(
                    and_(
                        ...
                        CompanyProspectEvidence.raw_snippet == evidence_snippet,
                        CompanyProspectEvidence.source_name.like(f"%{source_sha256}%") if source_sha256 else True,
                    )
                )
            )
            existing_evidence = existing_query.scalar_one_or_none()
